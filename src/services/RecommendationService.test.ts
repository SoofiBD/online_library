...
describe('RecommendationService', () => {
  it('fetches READ+rated profile books and WANT_TO_READ candidates, then delegates to the provider', async () => {
    const list = vi.fn(async (_ownerId: string, filter?: { status?: string }) => {
      if (filter?.status === 'READ') return [book({ id: 'read1', rating: 5 })]
